...
- [x] Move this plan to `docs/plans/completed/`. **Accepted with known gaps by explicit maintainer decision (2026-07-03):** only steps 2-5 of the Post-Completion checklist were actually confirmed in a live session; steps 6-12 (Esc, narrow terminal, dangerous/catastrophic multi-line render, Kitty vs. legacy protocol, Ctrl+C, PageUp/PageDown, `/reload`) were explicitly *not* verified — some are impractical for the maintainer to check right now (e.g. switching terminal protocols). Maintainer chose to accept the branch as-is and handle any fallout from the unverified paths as regular bugfixes if/when they surface, rather than blocking completion on further manual testing. This is a deliberate risk acceptance, not a claim that those steps passed — do not read the unchecked items above as verified.
